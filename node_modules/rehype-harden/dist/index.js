...
export function harden({ defaultOrigin = "", allowedLinkPrefixes = [], allowedImagePrefixes = [], blockedImageClass = "inline-block bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-400 px-3 py-1 rounded text-sm", blockedLinkClass = "text-gray-500", }) {
    ...
    return (tree) => {
        const visitor = createVisitor(defaultOrigin, allowedLinkPrefixes, allowedImagePrefixes, blockedImageClass, blockedLinkClass);
        visit(tree, visitor);
    };
}
...
function transformUrl(url, allowedPrefixes, defaultOrigin) {
    if (!url)
        return null;
...
const createVisitor = (defaultOrigin, allowedLinkPrefixes, allowedImagePrefixes, blockedImageClass, blockedLinkClass) => {
    const visitor = (node, index, parent) => {
        ...
        if (node.tagName === "a") {
            const transformedUrl = transformUrl(node.properties.href, allowedLinkPrefixes, defaultOrigin);
        ...
        if (node.tagName === "img") {
            const transformedUrl = transformUrl(node.properties.src, allowedImagePrefixes, defaultOrigin);
            if (transformedUrl === null) {
                // @ts-expect-error
                node[SEEN] = true;
                visit(node, visitor);
